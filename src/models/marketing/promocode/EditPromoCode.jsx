...
const EditPromoCode = ({ isOpen, onClose, promoCodeId }) => {
  ...
  const {
    ...
    queryKey: ["promo-code-detail", promoCodeId],
    queryFn: ({ queryKey }) => {
      const [, id] = queryKey;
      return fetchPromoCodeDetail(id, navigate);
    },
    enabled: isOpen,
  });
  ...
  useEffect(() => {
    promoCodeData && setFormData(promoCodeData);
  }, [promoCodeData]);
  ...
  const handleEditPromoCode = useMutation({
    ...
    onSuccess: () => {
      ...
      toaster.create({
        title: "Success",
        description: "Updated promo code detail",
        type: "success",
      });
    },
    onError: () => {
      toaster.create({
        title: "Error",
        description: "Error in updating promo detail",
        type: "error",
      });
    },
  });
  ...
  const showLoading = promoLoading || merchantLoading || geofenceLoading;
  const showError = promoError || merchantError || geofenceError;
