...
import {
  fetchSingleAppBanner,
  updateAppBannerDetail,
...
const EditAppBanner = ({ isOpen, onClose, bannerId }) => {
  ...
  const handleEditBanner = useMutation({
    mutationKey: ["edit-app-banner"],
    mutationFn: ({ bannerId, formDataObject }) =>
      updateAppBannerDetail(bannerId, formDataObject, navigate),
    onSuccess: () => {
      ...
      toaster.create({
        title: "Success",
        description: "Updated App Banner detail",
        type: "success",
      });
    },
    onError: () => {
      toaster.create({
        title: "Error",
        description: "Error in updating app banner detail",
        type: "error",
      });
    },
  });
